...
def write_water_color_variants(out_dir: Path) -> None:
    ...
    slate_deep = np.array([0.075, 0.115, 0.170], dtype=np.float32)
    slate_mid = np.array([0.150, 0.235, 0.330], dtype=np.float32)
    slate_light = np.array([0.290, 0.410, 0.545], dtype=np.float32)

    storm_deep = np.array([0.085, 0.095, 0.110], dtype=np.float32)
    storm_mid = np.array([0.175, 0.195, 0.230], dtype=np.float32)
    storm_light = np.array([0.315, 0.350, 0.410], dtype=np.float32)

    mint_deep = np.array([0.095, 0.215, 0.175], dtype=np.float32)
    mint_mid = np.array([0.190, 0.445, 0.365], dtype=np.float32)
    mint_light = np.array([0.345, 0.700, 0.585], dtype=np.float32)

    seaweed_deep = np.array([0.040, 0.115, 0.055], dtype=np.float32)
    seaweed_mid = np.array([0.085, 0.245, 0.120], dtype=np.float32)
    seaweed_light = np.array([0.170, 0.430, 0.220], dtype=np.float32)
    ...
    lavender_deep = np.array([0.165, 0.140, 0.240], dtype=np.float32)
    lavender_mid = np.array([0.340, 0.295, 0.500], dtype=np.float32)
    lavender_light = np.array([0.560, 0.520, 0.735], dtype=np.float32)

    plum_deep = np.array([0.120, 0.060, 0.145], dtype=np.float32)
    plum_mid = np.array([0.260, 0.125, 0.315], dtype=np.float32)
    plum_light = np.array([0.430, 0.220, 0.500], dtype=np.float32)
    ...
    rose_deep = np.array([0.220, 0.090, 0.130], dtype=np.float32)
    rose_mid = np.array([0.470, 0.195, 0.275], dtype=np.float32)
    rose_light = np.array([0.710, 0.330, 0.455], dtype=np.float32)

    sunset_deep = np.array([0.275, 0.120, 0.095], dtype=np.float32)
    sunset_mid = np.array([0.575, 0.260, 0.205], dtype=np.float32)
    sunset_light = np.array([0.860, 0.460, 0.370], dtype=np.float32)

    peach_deep = np.array([0.290, 0.170, 0.110], dtype=np.float32)
    peach_mid = np.array([0.610, 0.340, 0.230], dtype=np.float32)
    peach_light = np.array([0.885, 0.570, 0.400], dtype=np.float32)
    ...
    obsidian_deep = np.array([0.020, 0.028, 0.040], dtype=np.float32)
    obsidian_mid = np.array([0.050, 0.070, 0.095], dtype=np.float32)
    obsidian_light = np.array([0.105, 0.135, 0.180], dtype=np.float32)
    ...
    base_slate = remap_rgb_to_palette(base_rgb, slate_deep, slate_mid, slate_light, brightness_target=1.05, saturation=0.95)
    detail_slate = remap_rgb_to_palette(detail_rgb, slate_deep, slate_mid, slate_light, brightness_target=1.05, saturation=0.95)
    base_storm = remap_rgb_to_palette(base_rgb, storm_deep, storm_mid, storm_light, brightness_target=1.02, saturation=0.88)
    detail_storm = remap_rgb_to_palette(detail_rgb, storm_deep, storm_mid, storm_light, brightness_target=1.02, saturation=0.88)
    base_mint = remap_rgb_to_palette(base_rgb, mint_deep, mint_mid, mint_light, brightness_target=1.12, saturation=1.05)
    detail_mint = remap_rgb_to_palette(detail_rgb, mint_deep, mint_mid, mint_light, brightness_target=1.12, saturation=1.05)
    base_seaweed = remap_rgb_to_palette(base_rgb, seaweed_deep, seaweed_mid, seaweed_light, brightness_target=1.02, saturation=1.00)
    detail_seaweed = remap_rgb_to_palette(detail_rgb, seaweed_deep, seaweed_mid, seaweed_light, brightness_target=1.02, saturation=1.00)
    ...
    base_lavender = remap_rgb_to_palette(base_rgb, lavender_deep, lavender_mid, lavender_light, brightness_target=1.10, saturation=0.99)
    detail_lavender = remap_rgb_to_palette(detail_rgb, lavender_deep, lavender_mid, lavender_light, brightness_target=1.10, saturation=0.99)
    base_plum = remap_rgb_to_palette(base_rgb, plum_deep, plum_mid, plum_light, brightness_target=1.06, saturation=1.00)
    detail_plum = remap_rgb_to_palette(detail_rgb, plum_deep, plum_mid, plum_light, brightness_target=1.06, saturation=1.00)
    ...
    base_rose = remap_rgb_to_palette(base_rgb, rose_deep, rose_mid, rose_light, brightness_target=1.10, saturation=1.03)
    detail_rose = remap_rgb_to_palette(detail_rgb, rose_deep, rose_mid, rose_light, brightness_target=1.10, saturation=1.03)
    base_sunset = remap_rgb_to_palette(base_rgb, sunset_deep, sunset_mid, sunset_light, brightness_target=1.12, saturation=1.05)
    detail_sunset = remap_rgb_to_palette(detail_rgb, sunset_deep, sunset_mid, sunset_light, brightness_target=1.12, saturation=1.05)
    base_peach = remap_rgb_to_palette(base_rgb, peach_deep, peach_mid, peach_light, brightness_target=1.14, saturation=1.04)
    detail_peach = remap_rgb_to_palette(detail_rgb, peach_deep, peach_mid, peach_light, brightness_target=1.14, saturation=1.04)
    ...
    base_obsidian = remap_rgb_to_palette(base_rgb, obsidian_deep, obsidian_mid, obsidian_light, brightness_target=0.92, saturation=0.90)
    detail_obsidian = remap_rgb_to_palette(detail_rgb, obsidian_deep, obsidian_mid, obsidian_light, brightness_target=0.92, saturation=0.90)
    ...
    save_rgba(out_dir / "tile-water-base-slate.png", base_slate[..., 0], base_slate[..., 1], base_slate[..., 2], base_a)
    save_rgba(out_dir / "tile-water-detail-slate.png", detail_slate[..., 0], detail_slate[..., 1], detail_slate[..., 2], detail_a)
    save_rgba(out_dir / "tile-water-base-storm.png", base_storm[..., 0], base_storm[..., 1], base_storm[..., 2], base_a)
    save_rgba(out_dir / "tile-water-detail-storm.png", detail_storm[..., 0], detail_storm[..., 1], detail_storm[..., 2], detail_a)
    save_rgba(out_dir / "tile-water-base-mint.png", base_mint[..., 0], base_mint[..., 1], base_mint[..., 2], base_a)
    save_rgba(out_dir / "tile-water-detail-mint.png", detail_mint[..., 0], detail_mint[..., 1], detail_mint[..., 2], detail_a)
    save_rgba(out_dir / "tile-water-base-seaweed.png", base_seaweed[..., 0], base_seaweed[..., 1], base_seaweed[..., 2], base_a)
    save_rgba(out_dir / "tile-water-detail-seaweed.png", detail_seaweed[..., 0], detail_seaweed[..., 1], detail_seaweed[..., 2], detail_a)
    ...
    save_rgba(out_dir / "tile-water-base-lavender.png", base_lavender[..., 0], base_lavender[..., 1], base_lavender[..., 2], base_a)
    save_rgba(out_dir / "tile-water-detail-lavender.png", detail_lavender[..., 0], detail_lavender[..., 1], detail_lavender[..., 2], detail_a)
    save_rgba(out_dir / "tile-water-base-plum.png", base_plum[..., 0], base_plum[..., 1], base_plum[..., 2], base_a)
    save_rgba(out_dir / "tile-water-detail-plum.png", detail_plum[..., 0], detail_plum[..., 1], detail_plum[..., 2], detail_a)
    ...
    save_rgba(out_dir / "tile-water-base-rose.png", base_rose[..., 0], base_rose[..., 1], base_rose[..., 2], base_a)
    save_rgba(out_dir / "tile-water-detail-rose.png", detail_rose[..., 0], detail_rose[..., 1], detail_rose[..., 2], detail_a)
    save_rgba(out_dir / "tile-water-base-sunset.png", base_sunset[..., 0], base_sunset[..., 1], base_sunset[..., 2], base_a)
    save_rgba(out_dir / "tile-water-detail-sunset.png", detail_sunset[..., 0], detail_sunset[..., 1], detail_sunset[..., 2], detail_a)
    save_rgba(out_dir / "tile-water-base-peach.png", base_peach[..., 0], base_peach[..., 1], base_peach[..., 2], base_a)
    save_rgba(out_dir / "tile-water-detail-peach.png", detail_peach[..., 0], detail_peach[..., 1], detail_peach[..., 2], detail_a)
    ...
    save_rgba(out_dir / "tile-water-base-obsidian.png", base_obsidian[..., 0], base_obsidian[..., 1], base_obsidian[..., 2], base_a)
    save_rgba(out_dir / "tile-water-detail-obsidian.png", detail_obsidian[..., 0], detail_obsidian[..., 1], detail_obsidian[..., 2], detail_a)
...
def main() -> int:
    ...
    print("[watergen] wrote tile-water-base/detail variants: GREEN, BROWN, AMBER, YELLOW, ORANGE, LIME, TEAL, TURQUOISE, CYAN, SLATE, STORM, MINT, SEAWEED, JADE, LAVENDER, PLUM, PURPLE, VIOLET, MAGENTA, PINK, ROSE, SUNSET, PEACH, CORAL, OBSIDIAN, RED")
