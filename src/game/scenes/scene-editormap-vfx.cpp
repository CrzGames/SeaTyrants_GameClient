...
void EditorMapVfxScene::drawHud(void) const
{
    ...
    if (shipMode)
    {
        this->drawToolbarButton(this->buttonImportShipRect, "IMPORTER DOSSIER NAVIRES", false);
        this->drawToolbarButton(this->buttonImportSfxRect, "IMPORTER DOSSIER VFX", false);
        this->drawToolbarButton(this->buttonShipOrderMinusRect, "SHIP ORD -", false);
        this->drawToolbarButton(this->buttonShipOrderPlusRect, "SHIP ORD +", false);
        this->drawToolbarButton(this->buttonVfxOrderMinusRect, "VFX ORD -", false);
        this->drawToolbarButton(this->buttonVfxOrderPlusRect, "VFX ORD +", false);
        ...
        this->drawToolbarButton(
            this->buttonFlipHorizontalRect,
            "FLIP H",
            this->selectedVfxInstanceIndex >= 0 &&
                this->shipVfxInstances[static_cast<size_t>(this->selectedVfxInstanceIndex)].flipHorizontal);
        this->drawToolbarButton(
            this->buttonFlipVerticalRect,
            "FLIP V",
            this->selectedVfxInstanceIndex >= 0 &&
                this->shipVfxInstances[static_cast<size_t>(this->selectedVfxInstanceIndex)].flipVertical);
        this->drawToolbarButton(this->buttonRemoveVfxRect, "SUPPRIMER VFX", false);

        const RC2D_Color fpsFillColor = this->vfxFpsInputFocused
            ? RC2D_Color{58, 88, 122, 210}
            : RC2D_Color{28, 38, 50, 205};
        const RC2D_Color fpsBorderColor = this->vfxFpsInputFocused
            ? RC2D_Color{124, 186, 236, 245}
            : RC2D_Color{108, 126, 148, 220};
        rc2d_graphics_setBlendMode(RC2D_BLENDMODE_BLEND);
        rc2d_graphics_setColor(fpsFillColor);
        rc2d_graphics_rectangle("fill", &this->buttonVfxFpsInputRect);
        rc2d_graphics_setColor(fpsBorderColor);
        rc2d_graphics_rectangle("line", &this->buttonVfxFpsInputRect);
        rc2d_graphics_setBlendMode(RC2D_BLENDMODE_NONE);

        std::string fpsValue;
        if (this->vfxFpsInputFocused)
        {
            fpsValue = this->vfxFpsInput;
        }
        else if (this->selectedVfxInstanceIndex >= 0 &&
                 this->selectedVfxInstanceIndex < static_cast<int>(this->shipVfxInstances.size()))
        {
            fpsValue = formatSfxFpsValue(
                this->shipVfxInstances[static_cast<size_t>(this->selectedVfxInstanceIndex)].fps);
        }
        else if (this->selectedSfxIndex >= 0 &&
                 this->selectedSfxIndex < static_cast<int>(this->importedSfx.size()))
        {
        ...
        std::string fpsLabel = "Frame/S: " + fpsValue;
        if (this->vfxFpsInputFocused)
        {
            fpsLabel += "_";
        }
        ...
        rc2d_graphics_drawText(&fpsInputText, this->buttonVfxFpsInputRect.x + 6.0f, this->buttonVfxFpsInputRect.y + 2.0f);
...
bool EditorMapVfxScene::handleToolbarClick(float x, float y)
{
    ...
    if (this->editorMode == EditorMode::SHIP_VFX)
    {
        ...
        if (this->pointInRect(x, y, this->buttonShipOrderMinusRect))
        {
            this->adjustShipDrawOrder(-1);
            return true;
        }
        if (this->pointInRect(x, y, this->buttonShipOrderPlusRect))
        {
            this->adjustShipDrawOrder(1);
            return true;
        }
        if (this->pointInRect(x, y, this->buttonVfxOrderMinusRect))
        {
            this->adjustSelectedVfxDrawOrder(-1);
            return true;
        }
        if (this->pointInRect(x, y, this->buttonVfxOrderPlusRect))
        {
            this->adjustSelectedVfxDrawOrder(1);
            return true;
        }
        ...
        if (this->pointInRect(x, y, this->buttonRemoveVfxRect))
        {
            this->removeSelectedVfxInstance();
            return true;
        }
        if (this->pointInRect(x, y, this->buttonVfxFpsInputRect))
        {
            float currentFps = kLoosePreviewFpsDefault;
            if (this->selectedVfxInstanceIndex >= 0 &&
                this->selectedVfxInstanceIndex < static_cast<int>(this->shipVfxInstances.size()))
            {
                currentFps = this->shipVfxInstances[static_cast<size_t>(this->selectedVfxInstanceIndex)].fps;
            }
            else if (this->selectedSfxIndex >= 0 &&
                     this->selectedSfxIndex < static_cast<int>(this->importedSfx.size()))
            {
                currentFps = this->importedSfx[static_cast<size_t>(this->selectedSfxIndex)].defaultFps;
            }
            this->vfxFpsInput = formatSfxFpsValue(currentFps);
            this->vfxFpsInputFocused = true;
            this->loosePreviewFpsInputFocused = false;
            return true;
        }
    }
