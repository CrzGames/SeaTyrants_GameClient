...
MinimapWorldMapButtonWidget::MinimapWorldMapButtonWidget(void)
    : iconImage{},
      iconImageData{}
{
}
...
void MinimapWorldMapButtonWidget::load(void)
{
    this->iconImage = LoadStorageImage(
        "assets/images/ui-scene-game/icon-worldmap.png",
        RC2D_STORAGE_TITLE);
...
void MinimapWorldMapButtonWidget::unload(void)
{
    ResetStorageImageDataRef(&this->iconImageData);
    ResetStorageImageRef(&this->iconImage);
}

SDL_FRect MinimapWorldMapButtonWidget::getCurrentRect(const MinimapWidget& minimapWidget) const
{
    ...
    return SDL_FRect{
        ...
        minimapRect.y - (height * 0.5f),
        width,
        height
    };
}

void MinimapWorldMapButtonWidget::draw(const MinimapWidget& minimapWidget) const
{
...
bool MinimapWorldMapButtonWidget::containsPoint(float x, float y, const MinimapWidget& minimapWidget) const
{
    return pointInRect(x, y, this->getCurrentRect(minimapWidget));
}

HudCursorType MinimapWorldMapButtonWidget::getDesiredCursor(float x, float y, const MinimapWidget& minimapWidget) const
{
...
bool MinimapWorldMapButtonWidget::mousepressed(float x, float y, RC2D_MouseButton button, const MinimapWidget& minimapWidget) const
{
