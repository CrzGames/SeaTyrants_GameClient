...
bool getImageSize(const RC2D_ImageData& imageData, const RC2D_Image& image, float* outWidth, float* outHeight)
{
...
MinimapNavigationButtonWidget::MinimapNavigationButtonWidget(void)
    : iconImage{},
      iconImageData{}
{
}
...
void MinimapNavigationButtonWidget::load(void)
{
    this->iconImage = LoadStorageImage(
        "assets/images/ui-scene-game/icon-navigate.png",
        RC2D_STORAGE_TITLE);
...
void MinimapNavigationButtonWidget::unload(void)
{
    ResetStorageImageDataRef(&this->iconImageData);
    ResetStorageImageRef(&this->iconImage);
}

SDL_FRect MinimapNavigationButtonWidget::getCurrentRect(const MinimapWidget& minimapWidget) const
{
    ...
    return SDL_FRect{
        minimapRect.x + minimapRect.w - (width * 0.5f),
...
void MinimapNavigationButtonWidget::draw(const MinimapWidget& minimapWidget) const
{
...
bool MinimapNavigationButtonWidget::containsPoint(float x, float y, const MinimapWidget& minimapWidget) const
{
    return pointInRect(x, y, this->getCurrentRect(minimapWidget));
}

HudCursorType MinimapNavigationButtonWidget::getDesiredCursor(float x, float y, const MinimapWidget& minimapWidget) const
{
...
bool MinimapNavigationButtonWidget::mousepressed(float x, float y, RC2D_MouseButton button, const MinimapWidget& minimapWidget) const
{
