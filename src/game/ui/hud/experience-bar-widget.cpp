...
static constexpr float kHudStatusBarBottomMarginPx = 5.0f;
...
static constexpr RC2D_Color kTrackFill = RC2D_Color{16, 26, 34, 236};
static constexpr RC2D_Color kXpFill = RC2D_Color{12, 74, 104, 244};
static constexpr RC2D_Color kXpFillEdge = RC2D_Color{56, 122, 158, 248};
...
ExperienceBarWidget::ExperienceBarWidget(void)
    : labelFont{},
...
void ExperienceBarWidget::load(void)
{
...
void ExperienceBarWidget::unload(void)
{
    ResetStorageFontRef(&this->labelFont);
}

void ExperienceBarWidget::setCurrentExperiencePoints(int value)
{
    this->currentExperiencePoints = (std::max)(0, value);
}

void ExperienceBarWidget::setUiScale(float scale)
{
    this->uiScale = clampHudWidgetScale(scale);
}

void ExperienceBarWidget::setPositionOffset(float offsetX, float offsetY)
{
    this->positionOffset = SDL_FPoint{offsetX, offsetY};
}

SDL_FPoint ExperienceBarWidget::getPositionOffset(void) const
{
    return this->positionOffset;
}

void ExperienceBarWidget::resetPositionOffset(void)
{
    this->positionOffset = SDL_FPoint{0.0f, 0.0f};
}

SDL_FRect ExperienceBarWidget::getCurrentRect(void) const
{
    return this->getBarRect();
}

std::string ExperienceBarWidget::buildLabel(void) const
{
    return "XP : " + formatWithDots(this->currentExperiencePoints);
}

SDL_FRect ExperienceBarWidget::getBarRect(void) const
{
...
void ExperienceBarWidget::draw(void) const
{
    ...
    drawBeveledFilledRect(innerRect, kXpFill);

    SDL_FRect highlightRect = innerRect;
    highlightRect.h = (std::max)(1.0f, std::floor(innerRect.h * 0.35f));
    drawBeveledFilledRect(highlightRect, kXpFillEdge);
