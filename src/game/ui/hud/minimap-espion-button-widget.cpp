...
void MinimapEspionButtonWidget::load(void)
{
    this->iconImage = LoadStorageImage(
        "assets/images/ui-scene-game/icon-espion.png",
        RC2D_STORAGE_TITLE);
...
void MinimapEspionButtonWidget::unload(void)
{
    ResetStorageImageDataRef(&this->iconImageData);
    ResetStorageImageRef(&this->iconImage);
}

SDL_FRect MinimapEspionButtonWidget::getCurrentRect(const MinimapWidget& minimapWidget) const
{
...
void MinimapEspionButtonWidget::draw(const MinimapWidget& minimapWidget) const
{
...
bool MinimapEspionButtonWidget::containsPoint(float x, float y, const MinimapWidget& minimapWidget) const
{
    return pointInRect(x, y, this->getCurrentRect(minimapWidget));
}

HudCursorType MinimapEspionButtonWidget::getDesiredCursor(float x, float y, const MinimapWidget& minimapWidget) const
{
...
bool MinimapEspionButtonWidget::mousepressed(float x, float y, RC2D_MouseButton button, const MinimapWidget& minimapWidget) const
{
